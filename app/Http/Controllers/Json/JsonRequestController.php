...
class JsonRequestController extends Controller
{
    ...
    public function prescription_notf()
    {
        $data = Notification::where('prescription_id','!=',null)->where('is_read','=',0)->get()->count();
        return response()->json($data);            
    }

    public function prescription_notf_clear()
    {
        $data = Notification::where('prescription_id','!=',null);
        $data->delete();             
    }  

    public function order_notf()
    {
        $data = Notification::where('order_id','!=',null)->where('vendor_id',null)->where('is_read','=',0)->get()->count();
        return response()->json($data);            
    }
    public function order_notf_clear()
    {
        $data = Notification::where('order_id','!=',null)->where('vendor_id',null);
        $data->delete();             
    }
    
    public function vendor_order_notf()
    {
        $data = Notification::where('order_id','!=',null)->where('vendor_id','!=',null)->where('is_read','=',0)->get()->count();
        return response()->json($data);            
    }
    public function vendor_order_notf_clear()
    {
        $data = Notification::where('order_id','!=',null)->where('vendor_id','!=',null);
        $data->delete();             
    }
    ...
    public function vendor_product_notf()
    {
        $data = Notification::where('vendor_product_id','!=',null)->where('is_read','=',0)->get()->count();
        return response()->json($data);            
    }

    public function vendor_product_notf_clear()
    {
        $data = Notification::where('vendor_product_id','!=',null);
        $data->delete();             
    }  
    ...
    public function transhow(Request $request)
    {
        $id = $request->id;
    ...
    public function coupon()
    {
        ...
        if($fnd < 1)
        {
            ...
            if($from <= $today && $to >= $today)
            {
                ...
                {
                    $oldCart = Session::has('cart') ? Session::get('cart') : null;
                    $oldCoupon = Session::has('already') ? Session::get('already') : null;

                    $val = $oldCoupon ? $oldCoupon->code : null;
                    ...
                    $cc = CouponCheck::where('email','=',$user->email)->where('code', '=', $code)->exists();
                    ...
                    if($coupon->type == 0)
                    {
                        ...
                        $data[3] = $coupon->id;
                        $data[4] = $coupon->code." ".$coupon->price."%";
                        $data[5] = 1;
                        ...
                        $data[2] = round($sub * $curr->value, 2);
                        $data[3] = $coupon->id;
                        $data[4] = $curr->sign . round($coupon->price* $curr->value,2);
                        $data[5] = 1;    
                        
                        $coupon->discount = $sub;
                    }

                    Session::put('already', $coupon);                  
                    $data[0] = round($oldCart->total_with_discount()* $curr->value,2);
    ...
    {
        $id = $_GET['id'];
        $prod = Product::where('id','=',$id)->first(['id','user_id','adv_price','name','photo','size','color','cprice','pprice','stock','type','file','link','license','license_qty','measure','sale_from','sale_to','sale_percentage','sale_stock','purchase_limit']);
        
        // if($prod->requires_prescription){
        //     return response()->json(['error' => 'This product requires prescription and cannot be added to the cart.'], 422);
        // }

        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $totalQty = $cart->getQty($prod->id);

        $totalQty = ($totalQty > 0) ? $totalQty : 0;
        if($prod->purchase_limit && ($totalQty+1) > $prod->purchase_limit){
            return response()->json(['error' => 'Purchase Limit Exceeded'], 422);
        }

        $prod->price = $prod->getPrice($totalQty + 1);
        ...
        if($prod->user_id != 0){
            ...
            $discountPrice = $prod->price + $gs->fixed_commission + ($prod->price/100) * $gs->percentage_commission;
            $prod->price = round($discountPrice,2);
        }
    ...
    public function addbyone()
    {
        ...
        $data[4] = $cart->items[$id]['item']['price'];
    ...
    public function reducebyone()
    {
        ...
        if($totalQty > 1) $prod->price = $prod->getPrice($totalQty - 1);
        else $prod->price = $prod->getPrice(1);
    ...
    public function addnumcart()
    {
        ...
        $totalQty = ($totalQty > 0) ? $totalQty : 0;
        if($prod->purchase_limit && ($totalQty + $qty) > $prod->purchase_limit){
            return response()->json(['error' => 'Purchase Limit Exceeded'], 422);
        }

        $prod->price = $prod->getPrice($totalQty + $qty);
    ...
    public function addnumcartBonus()
    {
        $id = $_GET['id'];
        $qty = $_GET['qty'];
        $size = $_GET['size'];
        ...
        $prod->price = $prod->getPriceBonus($totalQty + $qty);
    ...
    {
        $search = $_GET['search'];     
        $data = Product::where('status','=',1)->where('name', 'like', '%' . $search . '%')->orWhere('company_name', 'like', '%' . $search . '%')->orWhere('tags', 'like', '%' . $search . '%')
                ->orderBy('name')->orderBy('id','desc')->take(20)->get();
        $product_count = Product::where('status','=',1)->where('name', 'like', '%' . $search . '%')->orWhere('company_name', 'like', '%' . $search . '%')->orWhere('tags', 'like', '%' . $search . '%')
        ->orderBy('name')->orderBy('id','desc')->get();
        $gs = Generalsetting::findOrFail(1);
        ...
        //     }
        // }
        $html = view('includes.search')->with('products',$data)->with('product_count',$product_count)->with('curr',$curr)->with('gs',$gs)->with('cart',$cart)->render();
        return response()->json(['html' => $html]);      
    }

    public function mobileSuggest()
    {
        $search = $_GET['search'];     
        $data = Product::where('status','=',1)->where('name', 'like', '%' . $search . '%')->orWhere('company_name', 'like', '%' . $search . '%')->orWhere('tags', 'like', '%' . $search . '%')
                ->orderBy('name')->orderBy('id','desc')->take(20)->get();
        ...
        //     }
        // }
        $html = view('includes.mobilesearch')->with('products',$data)->with('product_count',$product_count)->with('curr',$curr)->with('gs',$gs)->with('cart',$cart)->render();
        return response()->json(['html' => $html]);      
    }
    ...
    public function search()
    {
        ...
        foreach($data as $d){
            $d->cprice = $d->getPrice(1);
            $d->orig_stock = $d->getOriginal('stock');
        }
        return response()->json($data);      
    }
    ...
    public function sectionProducts()
    {
        ...
        foreach ($beproducts as $prod){
                    ...
                    $data .= '<a href="' . route('front.product', ['id' => $prod->id, 'slug' => str_slug($name,'-')]) . '" class="single-product-area text-center">
