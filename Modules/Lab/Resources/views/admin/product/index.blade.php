                            ...
                            <div class="row reorder-xs">
                                <div class="col-lg-6 col-md-5 col-sm-5 col-xs-12">
                                    <div class="product-header-title">
                                        <h2>All Lab Tests</h2>
                                        <p>Dashboard <i class="fa fa-angle-right" style="margin: 0 2px;"></i> Lab <i class="fa fa-angle-right" style="margin: 0 2px;"></i> All Tests</p>
                                    </div>
                                </div>
                                ...
                                <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                  <thead>
                                      <tr>
                                        ...
                                        {{-- <th style="width: 150px;">Test Category</th> --}}
                                        <th style="width: 150px;">Test Type</th>
                                        <th style="width: 100px;">Vendor Count</th>
                                  ...
                                  <tbody>
                                    ...
                                    @foreach($prods as $prod)    
                                        <tr>
                                          ...
                                          <td>{{ $prod->name }}<small style="display: block; color: #777;">ID: {{sprintf("%'.08d", $prod->id)}}</small></td>
                                          {{-- <td> {{ $prod->categories->cat_name }} </td> --}}
                                          <td> {{ $prod->type }} </td>
                                          <td> {{ $prod->options()->where('status',1)->count() }} </td>
                                          ...
                                          <td>                                         
                                            <span class="dropdown">
                                              ...
                                              <ul class="dropdown-menu">
                                                  <li><a href="{{route('lab-prod-st',['id1'=>$prod->id,'id2'=>1])}}">Active</a></li>
                                                  <li><a href="{{route('lab-prod-st',['id1'=>$prod->id,'id2'=>0])}}">Deactive</a></li>
                                              </ul>
                                            </span>
                                          </td>
                                          <td>

                                            <a href="{{route('lab-prod-edit',$prod->id)}}" class="btn btn-primary product-btn"><i class="fa fa-edit"></i> Edit</a>
                                            ...
                                            <a href="javascript:;" data-href="{{route('lab-prod-delete',$prod->id)}}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger product-btn"><i class="fa fa-trash"></i></a>
                                          </td>
                                        </tr>
                                    @endforeach
                                  </tbody>
  ...
  <script type="text/javascript">
          ...
          success:function(data){
            ...
            $("#feature .modal-content").append(''+
              '<form class="form-horizontal" action="{{url('/')}}/admin/product/feature/'+data[6]+'" method="POST">'+
                ...
                '<div class="modal-header">'+
                    '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+
                    '<h4 class="modal-title text-center" id="myModalLabel2">Products Title:'+data[7]+'</h4>'+
                '</div>'+
                '<div class="modal-body">'+
                        ...
                        '<div class="col-sm-9 col-sm-offset-3">'+
                          '<div class="btn btn-default checkbox1">'+
                              '<input type="checkbox" id="check1" name="featured" value="1" '+data[0]+'>'+ 
                                '<label for="check1">Add Products to {{$lang->bg}}</label>'+
                                  '</div>'+
                                  '</div>'+          
                                  '</div>'+
                                  '<div class="form-group">'+
                                ...
                                '<div class="col-sm-9 col-sm-offset-3">'+
                                    ...
                                    '<label for="chec2">Add Products to {{$lang->lm}}</label>'+
                                  '</div>'+
                                  '</div>'+
                                '</div>'+
                              '<div class="form-group">'+
                                ...
                                '<div class="col-sm-9 col-sm-offset-3">'+
                                  '<div class="btn btn-default checkbox1">'+
                                    '<input type="checkbox" id="chec3" name="top" value="1" '+data[2]+'>'+
                                      '<label for="chec3">Add Products to {{$lang->rds}}</label>'+
                                        '</div>'+
                                        '</div>'+
                                        '</div>'+
                                      '<div class="form-group">'+
                                          ...
                                          '<div class="col-sm-9 col-sm-offset-3">'+
                                            '<div class="btn btn-default checkbox1">'+
                                              '<input type="checkbox" id="check4" name="hot" value="1" '+data[3]+'>'+
                                                '<label for="check4">Add Products to {{$lang->hot_sale}}</label>'+
                                                    '</div>'+
                                                '</div>'+
                                            '</div>'+
                                            '<div class="form-group">'+
                                        ...
                                        '<label for="check5">Add Products to {{$lang->latest_special}}</label>'+
                                                    '</div>'+
                                                '</div>'+
                                            '</div>'+
                                            '<div class="form-group">'+
                                                ...
                                                '<label for="check6">Add Products to {{$lang->big_sale}}</label>'+
                                                  '</div>'+
                                                '</div>'+
                                            '</div>'+
                '</div>'+
    ...
    $( document ).ready(function() {
      $(".add-button").append('<div class="col-sm-4 add-product-btn text-right">'+
        '<a href="{{route('lab-prod-create')}}" class="add-newProduct-btn">'+
