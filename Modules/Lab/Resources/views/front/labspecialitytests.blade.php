...
@section('title','Lab Speciality Tests')
    ...
    <div class="row justify-content-between">
      <div class="col-lg-9 order-lg-2 g-mb-80">
        ...
        <div class="container g-mt-5">
         ...
         <ul class="breadcrumbs" style="margin-top: 10px;">
            ...
            <li class="last active"><a href="#"> Test For {{$spec[0]}}</a></li>
        </ul>
         ...
         {{-- <ul id="breadcrumbs">
          ...
          <ul class="nav u-nav-v2-1 u-nav-primary g-mb-20" role="tablist" data-target="nav-2-1-primary-hor-left" data-tabs-mobile-type="slide-up-down" data-btn-classes="btn btn-md btn-block rounded-0 u-btn-outline-primary g-mb-20">
            ...
            @foreach($specs->slice(0, 3) as $s)
            <li class="nav-item" style="margin-top:5px;margin-bottom:5px;">
              <a class="nav-link {{$loop->index==0?'active':''}}" data-toggle="tab" href="#{{$s->id}}" role="tab" style="border-radius: 30px; padding: 2px 10px;margin: 0px 5px;"> {{$s->name}}</a>
            </li>
          @endforeach
          ...
          <div class="collapse" id="collapseExample">
            ...
            <div id="nav-2-1-primary-hor-left" class="tab-content">
                ...
                @foreach($specs as $s)
                  <div class="tab-pane fade {{$loop->index==0?'show active':''}}" id="{{$s->id}}" role="tabpanel">
                    
                     
        
                        <div class="row">
                          @php
                                $prods = $s->options()->whereHas('vendor',function($q){
                            ...
                            <article class="u-shadow-v19 media g-bg-white rounded g-pa-20" data-animation="zoomIn" data-animation-delay="0" data-animation-duration="1000"><div class="media-body">
                                ...
                                <ul class="list-inline text-center" style="margin-bottom: 0rem;font-size:14px;">
                                  ...
                                  <div class=" g-font-size-12 g-brd-gray-light-v4 g-pa-15-20 text-center" >
                                    <input type="hidden" value="{{ $product->id }}" />
                                  <button class="btn btn-md u-btn-orange g-font-weight-600 g-font-size-11 text-uppercase" style="border-radius:30px;line-height:1; padding:0.57143rem 0.82857rem;" data-toggle="modal" data-target="#testdetails{{$s->id}}">
                      ...
                      <div class="modal fade" id="testdetails{{$s->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="exampleModalLabel">{{$product->vendor->name}}</h5>
      ...
      <div class="col-lg-3 order-lg-1 g-brd-right--lg g-brd-gray-light-v4 g-mb-80">
        <div class="container" style="">
          <!-- Links -->
          <div class="g-mb-5">
            <h4 class="h6 g-font-weight-700 g-mb-10 g-mt-0">Test By Speciality:</h4>
            <select id="mounth" class="form-control" onChange="window.location.href=this.value">
                <option value="hide">Search by Speciality</option>
                @foreach($labspecialities as $s)
                <option value="/lab/speciality/{{$s->speciality_slug}}" rel="icon-temperature" >{{$s->speciality_name}} ({{$s->products->count()}})</option>
