...
@section('title',$subcat->sub_name.' - Shop Category')
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-8 col-sm-12 col-xs-12">
                  <h1 class="h4">{{$subcat->sub_name}}</h1>
            
            <div class="container" style="margin: 10px;">
              @foreach($subcat->childs()->where('status','=',1)->orderBy('child_name')->get() as $childcategory)
              <a class="u-tags-v1 g-color-cyan g-brd-around g-brd-cyan g-bg-cyan-opacity-0_1 g-bg-cyan--hover g-color-white--hover g-rounded-50 g-py-4 g-px-15 g-mr-5 g-mb-7" style="padding-top:5px; padding-bottom:5px;" href="{{route('front.childcategory',$childcategory->child_slug)}}">{{ $childcategory->child_name }}</a>
              @endforeach
            </div>
                    ...
                    <div class="category-wrap">
                        <div class="row">
                            ...
                            @forelse($subcats->sortBy('priority_no') as $prod)
                                  @php
                      ...
                      @endphp
                            ...
                            @if(isset($max))  
                            @if($price < $max)
                            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12" style="width:50%;">
                                      @php
                          ...
                          <div class="product-description single-product-area text-center" style="margin-top:0px;">
                            <div class="product-name" style="margin-bottom: 0px;"><a href="{{route('front.product',['id' => $prod->id, 'slug' => str_slug($name,'-')])}}" class="text-center">{{strlen($prod->name) > 38 ? substr($prod->name,0,38)."..." : $prod->name}}</a></div>
                ...
                @if(isset($min) || isset($max))
                    <div class="row">
                        <div class="col-md-12 text-center"> 
                            {!! $subcats->appends(['min' => $min, 'max' => $max])->links() !!}               
                        </div>
                    </div>
                @else
                    <div class="row">
...
<script type="text/javascript">
        ...
        var sort = $("#sortby").val();
        window.location = "{{url('/subcategory')}}/{{$subcat->sub_slug}}/"+sort;
    });
</script>
