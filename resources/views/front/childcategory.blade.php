...
@section('title',$childcat->child_name.' - Shop Category')
@section('content')
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-9 col-md-8 col-sm-12 col-xs-12">
                  <h1 class="h4">{{$childcat->child_name}}</h1>
                    <div class="category-wrap">
                        <div class="row">
                                @forelse($childcats as $prod)
                                  @php
...
<div class="product-hover-area">
                    ...
                    <span class="wish-list hovertip wish-listt" data-toggle="modal" data-target="#myModal" rel-toggle="tooltip" title="{{$lang->quick_view}}"><i class="fa fa-eye"></i>
                          ...
                          <div class="product-description text-center single-product-area" style="margin-top:0px;">
                            <div class="product-name" style="margin-bottom: 35px;"><a href="{{route('front.product',['id' => $prod->id, 'slug' => str_slug($name,'-')])}}" class="text-center" style="margin-bottom:0px;">{{strlen($prod->name) > 38 ? substr($prod->name,0,38)."..." : $prod->name}}</a></div>
                    ...
                    @php
                                                    ...
                                                    @endphp 
                      <span style="display:inline; font-size:12px ;color:green;"><del class="offer-price" style="color:red;">{{$curr->sign}}{{round($pprice * $curr->value,2)}}</del> -{{ $prod->discount_percent }}%</span>  
                      @endif
...
<div class="product-hover-area">
                    ...
                    <span class="wish-list hovertip wish-listt" data-toggle="modal" data-target="#myModal" rel-toggle="tooltip" title="{{$lang->quick_view}}"><i class="fa fa-eye"></i>
                        ...
                        @endif
                          ...
                          <div class="product-description text-center single-product-area" style="margin-top:0px;">
                            ...
                            <div class="product-name" style="margin-bottom: 15px;"><a href="{{route('front.product',['id' => $prod->id, 'slug' => $name])}}" class=" text-center">{{strlen($prod->name) > 38 ? substr($prod->name,0,38)."..." : ucwords(strtolower($prod->name))}}</a>
                    ...
                    @if($prod->pprice != null && $prod->pprice != 0  && $prod->pprice > $prod->cprice)
                      <span style="display:inline; font-size:12px; color:green;"><del class="offer-price" style="color:red;">{{$curr->sign}}{{round($prod->pprice * $curr->value,2)}}</del> -{{ $prod->discount_percent }}%</span>  
                      @endif
                ...
                @if(isset($min) || isset($max))
                    <div class="row">
                        <div class="col-md-12 text-center"> 
                            {!! $childcats->appends(['min' => $min, 'max' => $max])->links() !!}               
                        </div>
                    </div>
                @else
                    <div class="row">
...
<script type="text/javascript">
        ...
        var sort = $("#sortby").val();
        window.location = "{{url('/childcategory')}}/{{$childcat->child_slug}}/"+sort;
    });
</script>
